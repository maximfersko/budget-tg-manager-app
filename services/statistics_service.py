...
class StatisticsService:
    ...
    async def get_base_stat(
        ...
        salary = \
        df_filtered[(df_filtered['raw_category'].str.lower() == 'зарплата') & (df_filtered['is_income'] == True)][
            'amount'].sum()
    ...
    async def get_categories_stat(self, repo: DBRepository, user_id: BigInteger, start_date=None,
                                  end_date=None) -> dict:
        operations: list[Operation] = await repo.get_user_operations(user_id)
        ...
        return {
            'top_expense_categories': {k: {'amount': float(round(v, 2)), 'percentage': float(
                round((v / total_expense) * 100, 2)) if total_expense > 0 else 0} for k, v in top_expense.items()},
            'top_income_categories': {k: {'amount': float(round(v, 2)), 'percentage': float(
                round((v / total_income) * 100, 2)) if total_income > 0 else 0} for k, v in top_income.items()}
        }
    ...
    def get_summary_for_ai(self, stats: dict, df: pd.DataFrame, is_category_filter: bool = False) -> str:
        ...
        top_cats = ", ".join([f"{k}: {v:.0f}" for k, v in
                              expense_df.groupby('raw_category')['amount'].sum().abs().sort_values(
                                  ascending=False).head(5).items()]) if not expense_df.empty else "none"

...
class StatisticsService:
    ...
    def _is_internal_transfer(self, row) -> bool:
        ...
        if category not in ('переводы', 'пополнения', 'пополнение'):
            return False
        return any(kw in description for kw in config.INTERNAL_TRANSFER_KEYWORDS)

    def _filter_internal_transfers(self, df: pd.DataFrame) -> pd.DataFrame:
        if df.empty:
            return df
        return df[df.apply(lambda row: not self._is_internal_transfer(row), axis=1)]

    @staticmethod
    def _to_dataframe(operations: List[Operation]) -> pd.DataFrame:
        if not operations:
    ...
    def _filter_statistics_date(
        self,
        operations: List[Operation],
        start_date: datetime,
        end_date: datetime,
    ) -> pd.DataFrame:
        df = self._to_dataframe(operations)
        if df.empty:
            return df
    ...
    @staticmethod
    async def _auto_detect_period(
        repo: DBRepository,
        user_id: int,
        start_date: datetime,
        end_date: datetime,
    ) -> tuple[datetime, datetime]:
        count = await repo.count_operations(user_id, start_date, end_date)
        if count > 0:
            return start_date, end_date
        last_date = await repo.get_last_operation_date(user_id)
        if not last_date:
            return start_date, end_date
        new_start = last_date.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
        new_end = (
            (new_start + timedelta(days=32)).replace(day=1) - timedelta(seconds=1)
        ).replace(microsecond=0)
        return new_start, new_end

    async def resolve_effective_dates(
        self,
        repo: DBRepository,
        user_id: int,
        start_date: Optional[datetime],
        ...
        if is_default:
            start_date, end_date = await self._auto_detect_period(repo, user_id, start_date, end_date)
        return start_date, end_date
    ...
    async def get_base_stat(
        ...
        operations = await repo.get_user_operations(user_id, start_date, end_date)
        df = self._to_dataframe(operations)
        ...
        result = {
            ...
            'balance': round(float(sum_income - sum_expense), 2),
            'avg_expense': round(float(expense_df['amount'].abs().mean() if not expense_df.empty else 0), 2),
    ...
    async def get_categories_stat(
        ...
        top_expense = (
            expense_df.groupby('raw_category')['amount'].sum().abs()
            .sort_values(ascending=False).head(10)
        )
        top_income = (
            income_df.groupby('raw_category')['amount'].sum()
            .sort_values(ascending=False).head(10)
        )
    ...
    def get_summary_for_ai(
        ...
        if not stats or stats.get('transactions_count', 0) == 0:
            return "No data."

        top_expense = cat_stats.get('top_expense_categories', {})
        top_cats = (
            ", ".join(f"{k}: {v['amount']:.0f}" for k, v in list(top_expense.items())[:5])
            or "none"
        )

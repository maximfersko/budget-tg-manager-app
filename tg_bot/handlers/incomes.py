...
async def process_bank_selection(callback_query: CallbackQuery, state: FSMContext):
    ...
    await callback_query.message.edit_text(
        text="Please send your CSV file"
    )
...
async def process_income_file(message: Message, state: FSMContext, bot: Bot):
    ...
    await message.answer(f"{message.from_user.first_name}, processing your file...")
    ...
    logger.info(f"Task {task.id} started for user {message.from_user.id}")

    await message.answer(
        f"File sent to queue\n"
        f"Task ID: {task.id}\n"
        f"You will get a notification when it's done."
    )
    await state.clear()
